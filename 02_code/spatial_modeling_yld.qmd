---
title: "Evaluating Integrated Soybean Cyst Nematode Management Using Spatially Informed Mixed Models"
subtitle: "Seed Yield Analysis"
date: "`r Sys.Date()`"
...
[GitHub Repository](https://github.com/LopezNicoraLab/SCN_spatial_inference)  
...
source(here("02_code","functions.r"))

cc <- colorRampPalette(rev(c("#6e7f6e", "#ccd5ae",  "#d4dcd5",  "#adcbe3", "#457b9d")))(15)

paper_categorical = c(
  "Fluopyram vs. base (sub-plot)" = "#4a5759", 
  "Peking vs. PI 88788 (whole plot)" = "#96abb1", 
  "Peking + base vs. PI 88788 + fluopyram (interaction)" = "#b5838d")


custom_colors = c("#4a5759","#b5838d")
...
```{r}
dat = read_csv(here("01_data","pioneers.csv"), show_col_types = FALSE)
...
dat = dat %>%
  mutate(
      ...
      log_centered_initial = log(SCN_initial) - mean(log(SCN_initial), na.rm = TRUE)
    ) %>%
...
p1 = dat %>%
  ...
  labs(
    ...
    x = expression("SCN egg population density at planting (" * italic(P)[italic(i)] * ")")
  )
...
p2 = dat %>%
  ...
  labs(
    y = expression("SCN reproduction factor (" * italic(Rf) == italic(P)[italic(f)] / italic(P)[italic(i)] * ")"),
    x = NULL,
    color = "Resistance source"
  ) + 
  guides(color = "none")  +
...
ggsave(here("03_outputs","fig_1.tiff"),units="in", width=7, height=9,bg = "white")
...
single_site_models = function(data, model_type, covariate = NULL) {
  ...
  model_expr = switch(model_type,
                      ...
                      "tps" = {
                         nseg = c(length(unique(data[["col"]])), length(unique(data[["row"]])))
                         TPXZg = tpsmmb("col", "row", data, stub = "1", nsegments = nseg,
                                         asreml = "grp", eigenvalues = "include")
...
write.csv(comparison_2022, file = here("03_outputs","model_selection", "comparison_2022.csv"), row.names = FALSE)
```
...
write.csv(seed_trt, file = here("03_outputs","seed_trt", "seed_trt_2022.csv"), row.names = FALSE)
...
write.csv(cult, file =here("03_outputs","cult_int", "cult_int_2022.csv"), row.names = FALSE)
...
write.csv(var_table, file = here("03_outputs","variances", "var_table_2022.csv"), row.names = FALSE)
...
write.csv(coef_damage, file = here("03_outputs","damage", "coef_damage_2022.csv"), row.names = FALSE)
...
write.csv(comparison_2023, file = here("03_outputs","model_selection", "comparison_2023.csv"), row.names = FALSE)
```
...
write.csv(seed_trt, file = here("03_outputs","seed_trt" , "seed_trt_2023.csv"), row.names = FALSE)
...
write.csv(cult, file = here("03_outputs","cult_int", "cult_int_2023.csv"), row.names = FALSE)
...
write.csv(var_table, file = here("03_outputs","variances", "var_table_2023.csv"), row.names = FALSE)
```
...
write.csv(coef_damage, file = here("03_outputs","damage","coef_damage_2023.csv"), row.names = FALSE)
...
write.csv(comparison_multi, file =  here("03_outputs","model_selection", "comparison_multi.csv"), row.names = FALSE)
```
...
write.csv(seed_trt, file = here("03_outputs","seed_trt", "seed_trt_multi.csv"), row.names = FALSE)
...
```{r,echo=FALSE,warning=FALSE,message=FALSE}
  ...
  write.csv(cult, file = here("03_outputs","cult_int", "cult_int_multi.csv"), row.names = FALSE)
...
write.csv(var_table, file = here("03_outputs","variances", "var_table_multi.csv"), row.names = FALSE)
...
write.csv(coef_damage, file = here("03_outputs","damage", "coef_damage_multi.csv"), row.names = FALSE)
...
trend_data <- bind_rows(
  tpsfit.2022 %>% mutate(year = 2022),
  tpsfit.2023 %>% mutate(year = 2023))


dat1 <- dat %>%
  left_join(trend_data, by = c("row", "col", "year")) %>%
  group_by(year) %>%
  mutate(mean_yield = mean(yield_kg_ha, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(yield_dev = yield_kg_ha - mean_yield)

global_range <- range(c(dat1$yield_dev, dat1$yield_dev), na.rm = TRUE)
nbreaks <- 15

unified_breaks <- round(seq(global_range[1], global_range[2], length.out = nbreaks), 0)

if (!0 %in% unified_breaks) {
  unified_breaks <- sort(c(0, unified_breaks))}


yield <- ggplot(dat1, aes(y = row, x = col, z = yield_dev)) +
  geom_contour_filled(breaks = unified_breaks, alpha = 0.9) +
  scale_fill_manual(
    values = cc, 
    name = expression(atop("Deviation from mean", atop("yield", paste("(kg ", ha^{-1}, ")")))),
    drop = FALSE
  ) +
  facet_grid(~year) +
  ...
  theme_minimal() +
  geom_rect(data = outer, aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax),
...
trend <- ggplot(dat1, aes(y = row, x = col, z = total)) +
  geom_contour_filled(breaks = unified_breaks, alpha = 0.9) +
  scale_fill_manual(
    values = cc, 
    name = expression(atop("Deviation from mean", atop("yield", paste("(kg ", ha^{-1}, ")")))),
...
ggsave(here("03_outputs","fig_2.tiff"), units = "in", width = 6, height = 5.5, bg = "white")

```
**Figure 2**. Spatial patterns of soybean yield deviation and fitted spatial trends across two growing seasons (2022 and 2023). **A**, Yield deviation (kg ha⁻¹) from the environment mean, showing observed spatial heterogeneity.**B**, Fitted spatial trend (kg ha⁻¹) from the tensor-product penalized spline (TPS) model, representing the estimated spatial field variation. Both panels use filled contour plots with consistent color scales. Black grid lines indicate individual plot boundaries, and outer rectangles mark the field boundaries for each year.
...
eff_data = rbind(eff_st_multi, eff_cult_multi) %>%
  ...
  mutate(
    Model = factor(Model, 
                   levels = c("scn_covariate", "ar1", "ar1_nugget", "ta", "tps"),
                   labels = c("SCN-adjusted", "AR1", "AR1 + nugget", "TA", "TPS")),
    Contrast = case_when(
      str_detect(Contrast, "Fluopyram vs Base") ~ "Fluopyram vs. base (sub-plot)",
      str_detect(Contrast, "Peking vs PI88788") ~ "Peking vs. PI 88788 (whole plot)",
      str_detect(Contrast, "Peking \\+ Base vs PI88788 \\+ Fluopyram") ~ "Peking + base vs. PI 88788 + fluopyram (interaction)",
      TRUE ~ NA_character_
    )
  )


ggplot(eff_data, aes(x = Model, y = Relative_Efficiency, group = Contrast, color = Contrast)) +
  geom_line(linewidth = 1, alpha = 0.8) +
  geom_point(size = 3) +
  geom_text_repel(
    data = filter(eff_data, Model == "TPS"),
    aes(label = paste0(round(Relative_Efficiency, 1), "%")),
    size = 4,
    nudge_x = 0.5,
    direction = "y",
    show.legend = FALSE
  ) +
  scale_color_manual(values = paper_categorical) +
  scale_y_continuous(
    name = expression(italic(RE) ~ "(%)"),
    limits = c(0, 55),
    breaks = seq(0, 50, 10),
    expand = c(0, 0)
  ) +
  labs(x = NULL, color = "Contrast") +
  theme(
...
ggsave(here("03_outputs","fig_4.tiff"), units = "in", width = 6, height = 5, bg = "white")

```
**Figure 4**. Relative efficiency of spatial models for estimating treatment contrasts. Relative efficiency (RE) is defined as the percent reduction in variance compared to a non-spatial baseline model (block + treatment effects only). Higher RE indicates greater precision gain. Three treatment contrasts are shown: Fluopyram vs. base (sub-plot), Peking vs. PI 88788 (whole-plot), and Peking + base vs. PI 88788 + fluopyram (sub-plot). Five spatial models were evaluated: SCN-adjusted (including pre-plant SCN density as a covariate), AR1, AR1 + nugget, TA (tensor-product additive), and TPS (tensor-product penalized spline). Model descriptions are provided in the Methods.
