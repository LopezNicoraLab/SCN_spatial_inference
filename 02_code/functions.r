...
spatial_matrix = function(data, x.coord.name = "col", y.coord.name = "row", at.name = "site") {
  ...
  multires = lapply(data1, function(dxy) {
    nseg = c(length(unique(dxy[[x.coord.name]])), length(unique(dxy[[y.coord.name]])))
    TPXZg = tpsmmb("col", "row", dxy, stub = "1", nsegments = nseg, asreml = "grp", eigenvalues = "include")
...
extract_logSCN_effect = function(model) {
  ...
  results = lapply(resis_names, function(coef_name) {
    if (coef_name %in% rownames(coefs)) {
      ...
      tibble(
        Resistance = sub("resis_", "", sub(":log_centered_initial", "", coef_name)),
        Estimate = est,
        StdError = se,
        PercentEffect = pct
      )
