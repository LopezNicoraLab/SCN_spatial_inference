...
subtitle: "SCN Reproduction Factor Analysis"
...
cc = colorRampPalette(c(  "#1d3557",  "#457b9d","#f1faee","#b5838d", "#6d597a" ))(12)
...
dat = dat %>%
  mutate(
      ...
      row_f = factor(row),
      col_f = factor(col),
      raw_SCN_rf = SCN_final/SCN_initial,
      log_SCN_rf = log(raw_SCN_rf)
    ) %>%
...
single_site_models = function(data, model_type) {
  ...
  fixed_formula = as.formula("log_SCN_rf ~ 1 + cult * seed_trt")
...
multi_site_models = function(data, model_type) {
  ...
  fixed_formula = as.formula("log_SCN_rf ~ 1 + site + cult * seed_trt")
  environment(fixed_formula) = baseenv()
...
write.csv(comparison_2022, file = here("03_outputs","model_selection", "comparison_2022_rf.csv"), row.names = FALSE)
```
...
write.csv(seed_trt, file = here("03_outputs","seed_trt","seed_trt_2022_rf.csv"), row.names = FALSE)
```
...
write.csv(cult, file = here("03_outputs","cult_int","cult_int_2022_rf.csv"), row.names = FALSE)
```
...
write.csv(var_table, file = here("03_outputs", "variances", "var_table_2022_rf.csv"), row.names = FALSE)
```
...
write.csv(comparison_2023, file =here("03_outputs", "model_selection", "comparison_2023_rf.csv"), row.names = FALSE)
```
...
write.csv(seed_trt, file = here("03_outputs","seed_trt", "seed_trt_2023_rf.csv"), row.names = FALSE)
```
...
write.csv(cult, file = here("03_outputs","cult_int","cult_int_2023_rf.csv"), row.names = FALSE)
```
...
write.csv(var_table, file = here("03_outputs","variances", "var_table_2023_rf.csv"), row.names = FALSE)
```
...
write.csv(comparison_multi, file = here("03_outputs","model_selection", "comparison_multi_rf.csv"), row.names = FALSE)
```
...
write.csv(seed_trt, file = here("03_outputs","seed_trt" ,"seed_trt_multi_rf.csv"), row.names = FALSE)
```
...
write.csv(cult, file = here("03_outputs","cult_int", "cult_int_multi_rf.csv"), row.names = FALSE)
```
...
write.csv(var_table, file = here("03_outputs","variances","var_table_multi_rf.csv"), row.names = FALSE)
```
...
dat1 <- dat %>%
  ...
  group_by(year) %>%
  mutate(mean_log_rf = mean(log_SCN_rf, na.rm = TRUE)) %>%
  ungroup() %>%
  mutate(log_SCN_rf_dev = log_SCN_rf - mean_log_rf)


global_range <- range(c(dat1$log_SCN_rf_dev, dat1$log_SCN_rf_dev), na.rm = TRUE)
nbreaks <- 12

unified_breaks <- round(seq(global_range[1], global_range[2], length.out = nbreaks), 1)

if (!0 %in% unified_breaks) {
  unified_breaks <- sort(c(0, unified_breaks))}


rf=ggplot(dat1, aes(y = row, x = col, z = log_SCN_rf_dev)) +
  geom_contour_filled(breaks = unified_breaks, alpha = 0.9) +
  scale_fill_manual(
    values = cc, 
    name = expression(atop("Deviation from mean", 
                   paste("ln(", italic(R)[italic(f)], ")"))),
    drop = FALSE
  ) +
  facet_grid(~year) +
...
ggsave(here("03_outputs","fig_3.tiff"), units = "in", width = 7, height = 5.5, bg = "white")
