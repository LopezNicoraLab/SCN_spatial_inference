...
rf=ggplot(dat1, aes(y = row, x = col, z = log_SCN_rf_dev)) +
  ...
  scale_fill_manual(
    values = cc, 
            name = expression(atop("Deviation in SCN reproduction", 
                           paste("factor [", italic(Rf) == log(italic(P)[italic(f)]/italic(P)[italic(i)]), "]"))),
    drop = FALSE
  ) +
  facet_grid(~year) +
...
ggsave("results/fig_3.tiff", units = "in", width = 7, height = 5.5, bg = "white")
